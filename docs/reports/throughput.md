...
- Workstation shared with other containers; absolute numbers may shift, but the
  qualitative conclusions (RTF cap ~4.5×, no multi-container scaling) are robust.
- 7-worker effective RTF unmeasured (sim barely started within 120 s).

## Next
- Benchmark **N-cars-in-one-world** (multi-robot namespacing) vs the single-instance
  ceiling. Then the Backlog-#3 auto-tuner over `(rtf × instances/cars)`.
